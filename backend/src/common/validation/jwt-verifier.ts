...
/** A token that has been verified, and when it stops being valid.
 *
 * `exp` is `undefined` on the remote path only: `supabase.auth.getUser()` answers
 * "is this token good right now" and does not hand back the claim set. A caller
 * that needs the expiry — a long-lived socket deciding when to hang up — has to
 * treat that as "unknown" rather than "never".
 */
...
/** Verifying a Supabase access token. **The whole trust boundary, in one place.**
 *
 * ⚠️ **Extracted from `JwtAuthGuard`, where it was private.** That made the guard
 * the only thing in the process that could verify a token — so anything else
 * needing one (a WebSocket handshake, a background job) would have written a
 * second implementation, and any divergence between two verifiers is a data
 * breach rather than a bug. There is one now.
 *
 * **Verification is local when `SUPABASE_JWT_SECRET` is set.** It used to always
 * call `supabase.auth.getUser(token)`, a network round trip to Supabase on *every*
 * authenticated request. Two problems with that:
 *
 * 1. **Latency.** Data lives in RDS inside the VPC, so Supabase is the only remote
 *    hop left in a request — it dominates the per-request budget for work that is
 *    a few microseconds of HMAC locally.
 * 2. **Availability.** When the Supabase project was recently PAUSED, *every*
 *    authenticated endpoint in production failed, not just login. Coupling every
 *    read in the app to a third party's uptime is the real bug; the latency is
 *    just the bill.
 *
 * When the secret is absent we fall back to the remote call, so nothing breaks
 * before the secret is configured in an environment. The active mode is logged
 * once at startup.
 */
@Injectable()
export class JwtVerifier {
  ...
  /** The user a token names, or `UnauthorizedException`. */
  async verify(token: string): Promise<User> {
    return (await this.verifyClaims(token)).user;
  }

  /** As `verify`, plus the `exp` the token carries.
   *
   * Separate rather than folded into `verify`'s return shape so the guard — which
   * has no use for `exp` — keeps the contract its tests already pin.
   */
  async verifyClaims(token: string): Promise<VerifiedToken> {
    return this.jwtSecret
      ? this.verifyLocally(token, this.jwtSecret)
      : { user: await this.verifyWithSupabase(token), exp: undefined };
  }
   ...
   * @returns the user carried by the token's claims, and its `exp`
   */
  private verifyLocally(token: string, secret: string): VerifiedToken {
    ...
    return { user: this.toUser(claims), exp: claims.exp };
  }
