...
- ~~**`onCreateWorkout` takes an `isTemplate` flag that is silently dropped.**~~ **Fixed, by deleting the flag.** It was a *wrong* requirement rather than a dropped one: it was passed `true` on the path that *applies* a template to an athlete, where the result is emphatically not a template, so honouring it would have inverted the feature. `onCreateWorkout` now takes **one object** instead of four positionals — a missing key is a hard `type-check` error, where the positional form let the arity mismatch through silently.
